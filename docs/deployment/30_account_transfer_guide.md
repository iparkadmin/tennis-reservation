...
| **GitHub** | vault モノレポ内（`tennis-reservation` はリポジトリルート直下） | 新アカウントで単独リポジトリ作成 |
...
```powershell
# モノレポ（vault）の Git ルートにいること
cd c:\Dev\vault
git subtree split -P tennis-reservation -b tennis-reservation-standalone
```
