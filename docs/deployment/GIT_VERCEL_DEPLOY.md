...
| **Root Directory** | **`tennis-reservation`**（GitHub の `TatsuhitoDT/vault` ではリポジトリルート直下） |
...
| ビルドが 404 / ファイルがない | Root Directory を **`tennis-reservation`** に設定しているか確認。詳細は `29_vercel_deploy_troubleshooting.md` |
| **No Next.js version detected** | Root Directory を **`tennis-reservation`**（またはトラブル時は `tennis-reservation/app`）に変更 → Redeploy |
