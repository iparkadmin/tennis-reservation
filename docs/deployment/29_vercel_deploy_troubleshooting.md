...
**推奨**: Root Directory を **`tennis-reservation/app`** に設定してください。

| 設定 | 値 |
|------|-----|
| Root Directory | `tennis-reservation/app` |
...
2. **Root Directory** を `tennis-reservation/app` に変更（`tennis-reservation` だけではなく **`/app` まで含める**）
...
`tennis-reservation` を Root にしている場合、`vercel.json` の `cd app && npm run build` が使われます。失敗する場合は上記「app を Root にする」設定を推奨します。

| リポジトリ構造 | Root Directory |
|----------------|----------------|
| `TatsuhitoDT/vault` のルート直下に `tennis-reservation` がある（現行） | `tennis-reservation` |
| 旧レイアウト（非推奨・履歴参照用） | `vault/tennis-reservation` |

**確認方法**: GitHub で [TatsuhitoDT/vault](https://github.com/TatsuhitoDT/vault) を開き、`tennis-reservation` がリポジトリルート直下にあるか確認。
