...
- **パスワードをリセット**しようとすると「このメールアドレスは登録されていません」と表示される（※アプリ対応済み：profiles チェックを廃止し、auth.users に存在すればリセットメールを送信）
...
- **パスワードリセット**：以前は `profiles` をチェックしていたが、RLS により未ログイン時は検索不可のため廃止。現在は `auth.users` に存在すればリセットメールを送信
