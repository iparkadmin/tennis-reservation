# iparkadmin への subtree push（リポジトリルートの .env.git.local の認証情報を使用）
# 使い方:
#   1. リポジトリルートの .env.git.example を .env.git.local にコピー
#   2. GITHUB_USERNAME と GITHUB_PAT を設定
#   3. .\push-iparkadmin.ps1

$ErrorActionPreference = "Stop"

# モノレポ（vault）リポジトリのルート = tennis-reservation/scripts から 2 階層上
...
if (-not (Test-Path $envFile)) {
    ...
    Write-Host "リポジトリルートで .env.git.example を .env.git.local にコピーし、GITHUB_USERNAME と GITHUB_PAT を設定してください。" -ForegroundColor Yellow
    exit 1
}
